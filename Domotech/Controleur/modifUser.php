...
if($_SERVER['REQUEST_METHOD'] === 'POST') {
 ...
 if(isset($_POST['btnModMdp'])){
   ...
   if(md5($_POST['prevMdp'])!=$infos['mdp']){ //md5(code): fonction qui hash le code utilisant le md5
    header ("Location: ../monespace.php?cible=monespace/monprofil.php&addel=lastMDP" );
    }
   else{
     if(($_POST['newMdp'])!=$_POST['confNewMdp']){
       header ("Location: ../monespace.php?cible=monespace/monprofil.php&addel=confMDP" );
     }
     else{ // si les mdp sont les memes
       $password = md5($_POST['newMdp']);
       $resultat = setMdp($db , $password , $_SESSION['userID']);
       echo ($resultat);
       header ("Location: ../monespace.php?cible=monespace/monprofil.php&addel=mdpOK" );
     }
...
function majMail(){
  ...
  echo ($resultat);
    header ("Location: ../monespace.php?cible=monespace/monprofil.php&addel=mail" );
}
...
function majTel(){
  ...
  echo ($resultat);
  header ("Location: ../monespace.php?cible=monespace/monprofil.php&addel=tel" );
}
